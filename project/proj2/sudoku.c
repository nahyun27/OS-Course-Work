 ...
 */
 /*
  * 2024-04-09, 컴퓨터학부, 2022058540 정헌희, check_rows, check_columns, check_subgrid, check_sudoku 함수 구현과 수정
  */
#include <stdio.h>
#include <stdlib.h>
#include <stdbool.h>
...
{
    /* sucess: sum({1,2,3,4,5,6,7,8,9}) = 45, fail: sum != 45 */
    int sum = 0;
    for(int i=0; i<9; i++){
        for(int j=0; j<9; j++){
            sum += sudoku[i][j];
        }
        if(sum == 45) valid[0][i] = true;
...
void *check_columns(void *arg)
{
    ...
    for(int i=0; i<9; i++){
        ...
        if(sum == 45) valid[1][i] = true;
        else valid[1][i] = false;
        sum = 0;
    }
}
...
void *check_subgrid(void *arg)
{
    ...
    if(sum == 45) valid[2][(int)n/3] = true;
    else valid[2][(int)n/3] = false;
}
...
void check_sudoku(void)
{
    ...
    pthread_t tid[11]; /* [0~8: subgrids], [9: rows], [10: columns] */
    /*
